#!/bin/bash
#SBATCH --job-name=GATK_CombineGenotypeGVCF_%j
...
#SBATCH --mem=48G
#SBATCH -t 3-0:0:0
#SBATCH -o GATK_CombineGenotypeGVCF_%j.out
#SBATCH -e GATK_CombineGenotypeGVCF_%j.out
...
--variant "$outDir"gvcfs.list -O "$outDir"BCGSC_OldReads_combined.g.vcf.gz \
-R "$ref" --tmp-dir "$SLURM_TMPDIR"

# Run GenotypeGVCFs
gatk --java-options "-Xmx4g" GenotypeGVCFs \
--variant "$outDir"BCGSC_OldReads_combined.g.vcf.gz -O "$outDir"BCGSC_OldReads_genotyped.vcf.gz -R "$ref" \
